...
class HostsAutomator:
    ...
    def input_hosts_details(self, discovered_hosts):
        ...
        while True:
            var = True
            host_options = input(
                "\033[1m Enter your choices(minimum 2 numbers comma separated): \033[0m").strip().split(',')
            for host_option in host_options:
                ...
                if var:
                    if int(host_option.strip()) not in range(1, len(serialno_to_thumbprint) + 1):
                        ...
                        var = False
            if var and len(host_options) < 2:
                self.utils.printRed("Please select atleast 2 other nodes as Minimum 3 nodes are required for initial "
                                    "configuration")
                var = False
            if var:
                break
        ...
        for host_serial_no in selected_hosts_serial_no:
            ...
            while True:
                ...
                if fqdn_to_serialno and fqdn in fqdn_to_serialno.keys():
                    self.utils.printRed("Input host FQDN already given for other hosts. Please pass different host FQDN")
                else:
                    break
